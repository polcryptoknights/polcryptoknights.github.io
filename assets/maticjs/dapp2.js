...
const contractAddress = "0xd88699ca3c46cc1321a16C83abE3f9469d7aF927"; //cryptoknights
...
async function loadAccount()
{
    ...
    console.log('POL Balance: ' + parseFloat(myBalance).toFixed(4));
...
async function deposit1() {
	...
	try{
		const sender = await contract.methods.MakeDeposit(spAccount,"75").send({from: defaultAccount, value: web3.utils.toWei($("#txtamount1").val(), "ether")})
...
async function deposit2()
{
	...
	try{
		const sender = await contract.methods.MakeDeposit(spAccount,"60").send({from: defaultAccount, value: web3.utils.toWei($("#txtamount2").val(), "ether")})
				.then(function(result){ 
					//location.reload();	
					loadContract();
	loadAccount();	
...
async function deposit3()
{
	...
	try{
		const sender = await contract.methods.MakeDeposit(spAccount,"50").send({from: defaultAccount, value: web3.utils.toWei($("#txtamount3").val(), "ether")})
				.then(function(result){ 
					//location.reload();	
					loadContract();
	loadAccount();	
